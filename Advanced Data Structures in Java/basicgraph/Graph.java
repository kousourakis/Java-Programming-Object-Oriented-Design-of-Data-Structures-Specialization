...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> degrSeq=new ArrayList<Integer>();
		for(int i=0;i<numVertices;i++){
			degrSeq.add(getNeighbors(i).size()+getInNeighbors(i).size());
		}
		for(int i=1;i<degrSeq.size();i++){
			int j=i;
			while(j>0 && degrSeq.get(j)>degrSeq.get(j-1)){
				degrSeq.set(j-1,degrSeq.set(j, degrSeq.get(j-1)));
				j--;
			}
		}
		return degrSeq;
